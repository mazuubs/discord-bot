...
const OWNER_ID = '1471476071290634305';
...
client.on('messageCreate', async (message) => {
  ...
  if (content === '+set pic') {
    ...
    if (!attachment) return message.reply('❌ Envoie une image avec la commande.');
    try {
      await client.user.setAvatar(attachment.url);
      await message.reply('✅ Photo de profil du bot mise à jour !');
    } catch (e) {
      await message.reply('❌ Impossible de changer la photo de profil (attends 10 min entre chaque changement).');
    }
  }

  else if (content === '+set banner') {
    const attachment = message.attachments.first();
    if (!attachment) return message.reply('❌ Envoie une image avec la commande.');
    try {
      await client.user.setBanner(attachment.url);
      await message.reply('✅ Bannière du bot mise à jour !');
    } catch (e) {
      await message.reply('❌ Impossible de changer la bannière.');
    }
  }
});
